...
namespace WEMBLEY.DemoApp.Core.Application.Services
{
    public class ApiService : IApiService
    {
        ...
        //Lay lastest ON OFF
        public async Task<IEnumerable<OperationStatusDto>> GetLastestOperation(string stationId)
        {
            HttpResponseMessage response = await _httpClient.GetAsync($"{serverUrl}/api/MachineStatuses/LatestOnOff?StationId={stationId}");

            response.EnsureSuccessStatusCode();
            string responseBody = await response.Content.ReadAsStringAsync();

            var result = JsonConvert.DeserializeObject<IEnumerable<OperationStatusDto>>(responseBody);
