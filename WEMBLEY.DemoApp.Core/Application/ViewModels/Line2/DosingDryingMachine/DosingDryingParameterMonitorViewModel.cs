...
namespace WEMBLEY.DemoApp.Core.Application.ViewModels.Line2.DosingDryingMachine
{
    public class DosingDryingParameterMonitorViewModel : BaseViewModel
    {
        ...
        public string HomeRefId { get; set; } = "";
        ...
        private async void LoadMFCMonitorViewAsync()
        {
            try
            {
                HomeRefId = _homeDataStore.LineReferences.FirstOrDefault(i => i.LineId == "NonVacuumBloodTube").ReferenceId;
            ...
            {
                ShowErrorMessage("Đã có lỗi xảy ra: Không tải được dữ liệu."); 
            }
            try
            {
