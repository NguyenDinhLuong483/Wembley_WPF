        ...
        private async void LoadDosingDryingMonitorView()
        {
            try
            {
                ...
                if (AllTags.Count != 0)
                {
                    ...
                    AllProductCount = Convert.ToInt64(await _signalRClient.GetBufferValue("IE-F3-BLO01", "productCount"));
                    PlasticTrayQuantity = Convert.ToInt64(await _signalRClient.GetBufferValue("IE-F3-BLO01", "S2_PLASTIC_TRAYS_QTY"));
                    isConnectPLC = Convert.ToInt64(await _signalRClient.GetBufferValue("IE-F3-BLO01", "isConnectPLC"));
            ...
            if (tag != null)
            {
                if (tag.StationId == "IE-F3-BLO01")
                {
                    switch (tag.TagId)
                    {
                        ...
                        //CAP voi TUBE dang lon bien voi nhau
                        case "S2_TUBE_DISABLE": CapEnable = (EStationEnable)Convert.ToInt32(tag.TagValue); break;
                        case "S2_CAP_DISABLE": TubeEnable = (EStationEnable)Convert.ToInt32(tag.TagValue); break;
